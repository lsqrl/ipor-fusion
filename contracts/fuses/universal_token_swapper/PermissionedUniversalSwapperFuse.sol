...
struct PermissionedUniversalSwapperData {
    address[] targets;
    bytes[] data;
}
...
struct PermissionedUniversalSwapperEnterData {
    address tokenIn;
    address tokenOut;
    uint256 amountIn;
    uint256 minAmountOut;
    PermissionedUniversalSwapperData data;
}
...
struct PermissionedSubstrateValidationResult {
    bool tokenInGranted;
...
/// @title PermissionedUniversalSwapperFuse
/// @notice Universal swapper that can only be executed via PlasmaVault.
/// @dev Executes in PlasmaVault storage context via delegatecall.
///      CRITICAL: This contract MUST NOT contain storage variables.
///      Slippage is configurable via substrate or defaults to DEFAULT_SLIPPAGE_WAD.
contract PermissionedUniversalSwapperFuse is IFuseCommon {
    using SafeERC20 for ERC20;

    /// @notice Emitted when entering a swap operation
    event PermissionedUniversalSwapperFuseEnter(
        address version,
        address tokenIn,
        address tokenOut,
    ...
    /// @notice Error thrown when caller is not the PlasmaVault execution context
    error PermissionedUniversalSwapperFuseOnlyPlasmaVault();
    /// @notice Error thrown when asset is not in the substrate configuration
    error PermissionedUniversalSwapperFuseUnsupportedAsset(address asset);
    /// @notice Error thrown when USD-based slippage check fails
    error PermissionedUniversalSwapperFuseSlippageFail();
    /// @notice Error thrown when minAmountOut is not reached
    error PermissionedUniversalSwapperFuseMinAmountOutNotReached(uint256 expected, uint256 actual);
    /// @notice Error thrown when price oracle returns zero price
    error PermissionedUniversalSwapperFuseInvalidPrice(address asset);
    /// @notice Error thrown when price oracle middleware is not configured
    error PermissionedUniversalSwapperFuseInvalidPriceOracleMiddleware();
    /// @notice Error thrown when amountIn is zero
    error PermissionedUniversalSwapperFuseZeroAmount();
    /// @notice Error thrown when marketId is zero
    error PermissionedUniversalSwapperFuseInvalidMarketId();
    /// @notice Error thrown when slippage exceeds 100%
    error PermissionedUniversalSwapperFuseSlippageExceeds100Percent(uint256 slippageWad);
    /// @notice Error thrown when targets array is empty
    error PermissionedUniversalSwapperFuseEmptyTargets();
    /// @notice Error thrown when targets and data arrays have different lengths
    error PermissionedUniversalSwapperFuseArrayLengthMismatch();
    ...
    /// @notice Creates a new PermissionedUniversalSwapperFuse instance
    /// @param marketId_ Market identifier for this fuse
    constructor(uint256 marketId_) {
        if (marketId_ == 0) {
            revert PermissionedUniversalSwapperFuseInvalidMarketId();
        }
    ...
    /// @param data_ Encoded PermissionedUniversalSwapperEnterData struct
    ...
    function enter(PermissionedUniversalSwapperEnterData calldata data_) external {
        _ensurePlasmaVaultExecution();

        if (data_.amountIn == 0) {
            revert PermissionedUniversalSwapperFuseZeroAmount();
        }

        uint256 dexsLength = data_.data.targets.length;
        if (dexsLength == 0) {
            revert PermissionedUniversalSwapperFuseEmptyTargets();
        }
        if (dexsLength != data_.data.data.length) {
            revert PermissionedUniversalSwapperFuseArrayLengthMismatch();
        }

        // Single pass substrate validation - reads substrates only once
        PermissionedSubstrateValidationResult memory validation = _validateSubstrates(
            data_.tokenIn,
            data_.tokenOut,
            data_.data.targets
        );
        ...
        if (!validation.tokenInGranted) {
            revert PermissionedUniversalSwapperFuseUnsupportedAsset(data_.tokenIn);
        }
        if (!validation.tokenOutGranted) {
            revert PermissionedUniversalSwapperFuseUnsupportedAsset(data_.tokenOut);
        }
        ...
        if (validation.grantedTargets != expectedMask) {
            ...
            for (uint256 i; i < dexsLength; ++i) {
                if ((validation.grantedTargets & (1 << i)) == 0) {
                    revert PermissionedUniversalSwapperFuseUnsupportedAsset(data_.data.targets[i]);
                }
            }
        }
        ...
        PermissionedBalances memory balances = PermissionedBalances({
        ...
        if (balances.tokenOutBalanceAfter <= balances.tokenOutBalanceBefore) {
            revert PermissionedUniversalSwapperFuseSlippageFail();
        }
        ...
        if (data_.minAmountOut > 0 && tokenOutDelta < data_.minAmountOut) {
            revert PermissionedUniversalSwapperFuseMinAmountOutNotReached(data_.minAmountOut, tokenOutDelta);
        }

        _validateUsdSlippage(data_.tokenIn, data_.tokenOut, tokenInDelta, tokenOutDelta, validation.slippageWad);

        emit PermissionedUniversalSwapperFuseEnter(VERSION, data_.tokenIn, data_.tokenOut, tokenInDelta, tokenOutDelta);
    }
    ...
    ) internal view returns (PermissionedSubstrateValidationResult memory result) {
        ...
        for (uint256 i; i < substratesLength; ++i) {
            ...
            if (UniversalTokenSwapperSubstrateLib.isTokenSubstrate(substrate)) {
                ...
                if (result.slippageWad > _ONE) {
                    revert PermissionedUniversalSwapperFuseSlippageExceeds100Percent(result.slippageWad);
                }
            }
        }
    ...
    /// @notice Ensures the call runs in PlasmaVault execution context
    function _ensurePlasmaVaultExecution() private view {
        if (!PlasmaVaultLib.isExecutionStarted()) {
            revert PermissionedUniversalSwapperFuseOnlyPlasmaVault();
        }
    }
}
