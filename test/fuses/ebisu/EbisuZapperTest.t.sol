...
contract EbisuZapperTest is Test {
    ...
    function testShouldEnterToEbisuZapper() public {
        // given
        EbisuZapperCreateFuseEnterData memory enterData = _buildEnterData(delegateWallet);
    ...
    function testShouldRegisterDelegateAndAllowDelegateToAdjustRate() public {
        EbisuZapperCreateFuseEnterData memory enterData = _buildEnterData(delegateWallet);

        uint256 troveId = _openTrove(enterData);

        IEbisuBorrowerOperationsHelperMinimal helper = IEbisuBorrowerOperationsHelperMinimal(
            IAddressesRegistry(SUSDE_REGISTRY).borrowerOperationsHelper()
        );
        (address account, uint128 minRate, uint128 maxRate, uint256 minChangePeriod) = helper
            .getInterestIndividualDelegateOf(troveId);

        IBorrowerOperations borrowerOpsFull = _getBorrowerOperations();
        ...
        assertEq(minChangePeriod, DELEGATE_MIN_CHANGE_PERIOD, "Delegate min change period mismatch");

        vm.warp(block.timestamp + DELEGATE_MIN_CHANGE_PERIOD + 1);

        vm.startPrank(delegateWallet);
        vm.expectRevert();
        borrowerOpsFull.adjustTroveInterestRate(troveId, DELEGATE_MAX_RATE + 1, 0, 0, 5 * 1e18);
        vm.stopPrank();
        ...
        ITroveManager troveManager = ITroveManager(ILeverageZapper(enterData.zapper).troveManager());
        ITroveManager.LatestTroveData memory troveData = troveManager.getLatestTroveData(troveId);
    ...
    function _buildEnterData(address delegate) private pure returns (EbisuZapperCreateFuseEnterData memory) {
        uint256 minRate = delegate == address(0) ? 0 : DELEGATE_MIN_RATE;
        uint256 maxRate = delegate == address(0) ? 0 : DELEGATE_MAX_RATE;
        uint256 minChangePeriod = delegate == address(0) ? 0 : DELEGATE_MIN_CHANGE_PERIOD;
        ...
        return
            EbisuZapperCreateFuseEnterData({
                ...
                interestDelegate: delegate,
                delegateMinInterestRate: minRate,
                delegateMaxInterestRate: maxRate,
                delegateMinInterestRateChangePeriod: minChangePeriod
            });
    }

    function _openTrove(EbisuZapperCreateFuseEnterData memory enterData) private returns (uint256 troveId) {
        deal(USDC, address(this), 100_000 * 1e6);
        ERC20(USDC).approve(address(plasmaVault), 100_000 * 1e6);
        plasmaVault.deposit(100_000 * 1e6, address(this));
        ...
        address wethEthAdapter = wethEthAdapterAddressReader.getEbisuWethEthAdapterAddress(address(plasmaVault));
        return EbisuMathLib.calculateTroveId(address(wethEthAdapter), address(plasmaVault), enterData.zapper, 1);
    }

    function _getBorrowerOperations() private view returns (IBorrowerOperations) {
        IBorrowerOperations borrowerOps = IAddressesRegistry(SUSDE_REGISTRY).borrowerOperations();
        return IBorrowerOperations(address(borrowerOps));
    }
